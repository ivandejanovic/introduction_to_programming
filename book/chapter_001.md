...
Now it is important to note here that machine instructions anuy computer can execute depends on the processor that computer is using. For that reason there are many "version" of machine instructions and conversly many versions of assembly.
...
That is why so called higher programming languages were invented. Higher order programing languages allow us to write more concise code and than those instructions get translated to machine instructions by either compiler or interpreter.
...
- Installed Golang (you can find instruction how to install Go on various operating systems on the official website)
